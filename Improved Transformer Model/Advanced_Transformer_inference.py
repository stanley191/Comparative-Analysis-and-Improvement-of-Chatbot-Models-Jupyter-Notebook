...
class TransformerChatbot:
    def __init__(self, model_path='advanced_transformer_chatbot_model.h5', tokenizer_path='tokenizer_data.pkl'):
    ...
    def batch_test(self, test_messages, size):
        """Test the chatbot with a batch of messages"""
        print("\nBatch Testing:")
        print("-" * 50)

        df = pd.DataFrame(columns=['Advanced Transformer', 'Predicted Intent', 'True intent', 'Confidence (0-1)', 'Relevance (0-1)', 'Response Time (s)']) # added true intent for easier analysis
        counter = 0
        for message in test_messages:
            ...
            new_line =({'Advanced Transformer': f'prompt {counter}',
            'Predicted Intent': intent,
            'True intent': message[1],
        ...
        if size == 'large':    
            df.to_csv('Adv_Transformer_large_test_data.csv', index=False)
        else:
            df.to_csv('Adv_Transformer_small_test_data.csv', index=False)
...
def main():
    ...
    ["Do you have an age?", "age"]
        ...
        messages_small = [
    ["Hi there", "greeting"],
    ["That's great! Can you help me with something?", "help"],
    ["What can you do exactly?", "capabilities"],
    ["Cool! Tell me a joke then", "joke"],
    ["Haha, that was good! Thanks a lot", "thanks"],
    ["Alright, I should get going now. Bye!", "goodbye"]
]
        chatbot.batch_test(messages_Large, size='large')
        chatbot.batch_test(messages_small, size='small')
        
    except FileNotFoundError as e:
        print(f"Error: Could not find required files. Please make sure you have:")
        print("1. advanced_transformer_chatbot_model.h5 (trained model)")
